...
'''
start_total = time.time()
for i in range(len(abt_list)-2,len(abt_list)):
	start_batch = time.time()
	p1 = subprocess.Popen("python retrain.py --model SWSModel --alpha {} --beta {} --tau {} --mixtures 8 --temp 8".format(abt_list[i][0], abt_list[i][1], abt_list[i][2]).split(" "))
